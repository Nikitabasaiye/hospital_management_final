...
import { useSelector } from "react-redux"; // For data fetch from the store
import { useDispatch } from "react-redux"; // For logout after the loggout
...
export default function Navbar({ navDark, manuClass, containerClass }) {
  ...
  const reduxUserType = useSelector((state) => state.auth.userType);
  const [userType, setUserType] = useState(reduxUserType);

  console.log({ userType: userType });
  const handleLogout = (e) => {
    console.log("Handle logout clicked:", e.target);
    if (profile?.data?.token) {
      dispatch(logout(profile.data.token));
    } else {
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 ms-2">
                    ...
                    <small className="text-muted">
                      {profile?.data?.specialty || "Speciality"}
                    </small>
                  </div>
                </Link>
                <Link
                ...
                <Link
                  ...
                  onClick={(e) => {
                    ...
                    handleLogout(e); // Call your logout logic
                  }}
                  // to="/login"
                >
                  <span className="mb-0 d-inline-block me-1">
            ...
            {userType === "patient" && (
              ...
              >
                ...
                <ul className="submenu">
                  ...
                  <li
                    className={manu === "booking-appointment" ? "active" : ""}
                  >
